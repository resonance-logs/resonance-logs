...
enum DirtyNode {
    ...
    Struct {
        type_id: i32,
        children: Vec<DirtyNode>,
    },
}

fn parse_dirty_stream(data: &[u8]) -> Vec<DirtyNode> {
    let mut nodes = Vec::new();
    let mut cursor = 0;

    while cursor + 8 <= data.len() {
        let val = i32::from_le_bytes(data[cursor..cursor + 4].try_into().unwrap());
        // Skip padding (bytes 4..8)

        if val == -2 {
            // Header: [Magic(-2), Size, Type]
            // We need at least 24 bytes: 0..8 (Magic), 8..16 (Size), 16..24 (Type)
            if cursor + 24 > data.len() {
                break;
            }

            let size = i32::from_le_bytes(data[cursor + 8..cursor + 12].try_into().unwrap());
            let type_id = i32::from_le_bytes(data[cursor + 16..cursor + 20].try_into().unwrap());

            let payload_start = cursor + 24;
            let payload_end = payload_start + (size as usize);

            if payload_end <= data.len() {
                let children = parse_dirty_stream(&data[payload_start..payload_end]);
                nodes.push(DirtyNode::Struct { type_id, children });
                cursor = payload_end;
            } else {
                cursor += 8; // Skip invalid header
            }
        } else if val == -3 {
            // Terminator
            cursor += 8;
            ...
            // Value
            nodes.push(DirtyNode::Value(val));
            cursor += 8;
        }
    }
    nodes
...
fn interpret_dungeon_data(nodes: &[DirtyNode]) {
    use tracing::info;

    for node in nodes {
        match node {
            DirtyNode::Struct { children, .. } => interpret_dungeon_data(children),
            DirtyNode::Value(_) => {}
        }
    }

    if nodes.len() == 3 {
        if let (DirtyNode::Value(k), DirtyNode::Value(v), DirtyNode::Value(ts)) =
            (&nodes[0], &nodes[1], &nodes[2])
        {
            if *ts > 1700000000 && *ts < 2000000000 {
                let dt = chrono::DateTime::from_timestamp(*ts as i64, 0)
                    ...
                    .unwrap_or_default();
                info!("Dungeon Update: Key {} -> {} (at {})", k, v, dt);
            }
        }
    }
}

fn interpret_container_data(nodes: &[DirtyNode]) {
    use tracing::info;

    let mut i = 0;
    while i < nodes.len() {
        match &nodes[i] {
            DirtyNode::Struct { type_id, children } => {
                if *type_id == 5 {
                    info!("  Item/Struct (Type 5): {} props", children.len());
                } else if *type_id == 1 {
                    info!("Container List (Size {})", children.len());
                    interpret_container_data(children);
                } else {
                    interpret_container_data(children);
                }
            }
            DirtyNode::Value(v) => {
                if i + 1 < nodes.len() {
                    if let DirtyNode::Struct {
                        type_id: 5,
                        children,
                    } = &nodes[i + 1]
                    {
                        info!("Item Update: ID {} ({} props)", v, children.len());
                        i += 1;
                    }
                }
            }
...
fn parse_dirty_wrapper(data: &[u8]) -> Vec<DirtyNode> {
    let mut payload = data;
    if !payload.is_empty() && payload[0] == 0x0a {
        let mut slice = &payload[1..];
        if let Ok(len) = prost::encoding::decode_varint(&mut slice) {
            let len_usize = len as usize;
            if slice.len() >= len_usize {
                payload = &slice[..len_usize];
            }
        }
    }
    parse_dirty_stream(payload)
}

fn visit_dirty_stream(data: &[u8], indent: usize, type_hint: Option<i32>) {
    use tracing::info;
    let indent_str = "  ".repeat(indent);
    let mut cursor = 0;
    // Store (value, sentinel) pairs
    let mut raw_buffer: Vec<i32> = Vec::new();

    // Helper to flush raw values
    let flush_raw = |buffer: &mut Vec<i32>, indent_s: &str, is_type_5: bool| {
        if !buffer.is_empty() {
            let mut output = String::new();
            output.push_str("[Values] ");

            // For Type 5 (likely items), the last value is often an ID
            let len = buffer.len();
            for (i, &v) in buffer.iter().enumerate() {
                ...
                // Timestamp heuristic
                if v > 1700000000 && v < 2000000000 {
                    if let Some(dt) = chrono::DateTime::from_timestamp(v as i64, 0) {
                        output.push_str(&format!("{} ({})", v, dt.format("%Y-%m-%d %H:%M:%S")));
                        continue;
                    }
                }

                // ID heuristic for Type 5 (last element)
                if is_type_5 && i == len - 1 {
                    output.push_str(&format!("ID:{}", v));
                    continue;
                }

                output.push_str(&v.to_string());
            }
            info!("{}{}", indent_s, output);
            buffer.clear();
        }
    };

    while cursor + 8 <= data.len() {
        // Read 8 bytes: Value (4) + Padding (4)
        let val_bytes = &data[cursor..cursor + 4];
        let pad_bytes = &data[cursor + 4..cursor + 8];

        let val = i32::from_le_bytes(val_bytes.try_into().unwrap());
        let pad = i32::from_le_bytes(pad_bytes.try_into().unwrap());

        // Check padding (Silent unless unexpected, DEADBEEF = -559038737)
        if pad != -559038737 {
            // warn!("{}[Warn: Non-standard padding: {}]", indent_str, pad);
        }

        // Header Start: -2
        if val == -2 {
            // Flush text values before starting a new node
            flush_raw(&mut raw_buffer, &indent_str, type_hint == Some(5));

            // Need 24 bytes total for header (3 * 8 bytes)
            if cursor + 24 > data.len() {
                info!("{}[Error: Incomplete Header at {}]", indent_str, cursor);
                break;
            }

            // Read Size (Next 8 bytes)
            let size_bytes = &data[cursor + 8..cursor + 12];
            let size = i32::from_le_bytes(size_bytes.try_into().unwrap());

            // Read Type (Next 8 bytes)
            let type_bytes = &data[cursor + 16..cursor + 20];
            let type_id = i32::from_le_bytes(type_bytes.try_into().unwrap());
            ...
            info!("{}[{} Type={} Size={}]", indent_str, label, type_id, size);
            ...
            if payload_end > data.len() {
                info!(
                    "{}  [Error: Header declares size {} but only {} bytes remain]",
                    indent_str,
                    size,
            ...
            // Recurse
            visit_dirty_stream(payload, indent + 1, Some(type_id));

            // Advance cursor past this node
            cursor = payload_end;
        } else if val == -3 {
            // Terminator (0xFFFFFFFD)
            flush_raw(&mut raw_buffer, &indent_str, type_hint == Some(5));
            // info!("{}[Terminator]", indent_str);
            cursor += 8;
        } else {
            // Raw value
            raw_buffer.push(val);
...
pub fn process_sync_container_dirty_data(
    ...
    let raw_bytes = sync_container_dirty_data.v_data.as_ref()?;
    info!("SyncContainerDirtyData (Len={}):", raw_bytes.len());

    let nodes = parse_dirty_wrapper(raw_bytes);
    interpret_container_data(&nodes);
...
pub fn process_sync_dungeon_dirty_data(
    ...
    let raw_bytes = sync_dungeon_dirty_data.v_data.as_ref()?;
    info!("SyncDungeonDirtyData (Len={}):", raw_bytes.len());

    let nodes = parse_dirty_wrapper(raw_bytes);
    interpret_dungeon_data(&nodes);
