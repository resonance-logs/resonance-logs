...
<div class="relative flex flex-col gap-2 overflow-hidden rounded-lg ring-1 ring-border/60 bg-card/30">
  ...
  <th class="px-3 py-2 text-[11px] text-left font-medium uppercase tracking-wide text-muted-foreground">Player</th>
        {#each visiblePlayerColumns as col (col.key)}
          <th class="px-3 py-2 text-[11px] text-right font-medium uppercase tracking-wide text-muted-foreground">{col.header}</th>
        {/each}
      </tr>
    </thead>
    <tbody>
      {#each healData as player (player.uid)}
        ...
        >
          ...
          {/each}
          <TableRowGlow className={className} percentage={SETTINGS.live.general.state.relativeToTopHealPlayer ? maxHeal > 0 ? (player.totalDmg / maxHeal) * 100 : 0 : player.dmgPct} />
        </tr>
      {/each}
    </tbody>
  </table>
